...
- `calibratedFaceBox` anchors the usable box to the detector's keypoint span (`keypointFaceBox`): the six Blaze landmarks sit on the face even when the raw detector rect drifts, so the derived box can never be displaced off the face. The raw box is only a fallback when keypoints are absent, and `keypointSpanDegenerate` rejects reads whose span is too small to describe a face. If screenshots show drift, tune only `keypointFaceBox`'s proportional margins — do not reintroduce large magic shifts: the old `-0.75w`/`-0.55h` shifts were what displaced the rect.
